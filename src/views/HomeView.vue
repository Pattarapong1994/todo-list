...
const changeOnStatus = async (event, todoId) => {
    isLoading.value = true
    try {
...
<template>
    <div>
        <div class="flex">
            <input class="input input-bordered w-full" type="text" placeholder="Input your activity....."
                v-model="todoText">
        ...
        <div class="flex items-center justify-between mt-3" v-for="todo in filterTodoList" :key="todo.id">
            <input type="checkbox" :checked="todo.status === 'Done'" class="checkbox"
                @change="changeOnStatus($event, todo.id)" />
            <div :class="todo.status === 'Done' ? 'line-through' : ''">
                {{ todo.name }}
            </div>
            <div>
                <RouterLink :to="{ name: 'todo-edit', params: { id: todo.id } }">
                    <button class="btn btn-square btn-outline"> <svg xmlns="http://www.w3.org/2000/svg" height="1em"
